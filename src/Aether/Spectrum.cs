...
namespace Aether
{
    public class Spectrum
    {
        ...
        private const int NumSamples = 3;
        ...
        public Spectrum(float[] coefficients)
        {
            if (coefficients.Length != NumSamples)
        ...
        /// <summary>
        /// Returns true if all this spectrum's coefficients are 0.
        /// </summary>
        public bool IsBlack
        {
            get { return _coefficients.All(x => x == 0.0f); }
        }
        ...
        /// <summary>
        /// Adds the values in a given spectrum to this spectrum.
        /// </summary>
        public void Add(Spectrum other)
        {
        ...
        /// <summary>
        /// Converts a spectrum (of RGB data) to XYZ data.
        /// </summary>
        public float[] ToXyz()
        {
            var xyz = new float[3];
            RgbToXyz(_coefficients, xyz);
            return xyz;
        }
        ...
        /// <summary>
        /// Converts XYZ data to RGB data.
        /// </summary>
        public static void XyzToRgb(float[] xyz, IList<float> rgb)
        {
        ...
        {
            var coefficients = new float[3];
            for (var i = 0; i < NumSamples; i++)
                coefficients[i] = MathUtility.Sqrt(spectrum._coefficients[i]);
            return new Spectrum(coefficients);
        }

        /// Creates a new spectrum, applying Exp for each coefficient.
        public static Spectrum Exp(Spectrum spectrum)
        {
            var coefficients = new float[3];
            for (var i = 0; i < NumSamples; i++)
                coefficients[i] = MathUtility.Exp(spectrum._coefficients[i]);
            return new Spectrum(coefficients);
        }

        /// Creates a new spectrum, applying Clamp for each coefficient.
        public static Spectrum Clamp(Spectrum spectrum, float low = 0, float high = Single.PositiveInfinity)
        {
            var coefficients = new float[3];
            for (var i = 0; i < NumSamples; i++)
                coefficients[i] = MathUtility.Clamp(spectrum._coefficients[i], low, high);
            return new Spectrum(coefficients);
        }

        private static float InterpolateSpectrumSamples(float[] lambda, float[] values, int n, float l)
        {
            for (var i = 0; i < n - 1; i++)
                Debug.Assert(lambda[i + 1] > lambda[i]);
            if (l <= lambda[0])
                return values[0];
            if (l >= lambda[n - 1])
                return values[n - 1];
            for (var i = 0; i < n - 1; ++i)
                ...
                {
                    var t = (l - lambda[i]) / (lambda[i + 1] - lambda[i]);
                    return MathUtility.Lerp(t, values[i], values[i + 1]);
                }
            throw new InvalidOperationException();
        }

        /// <summary>
        /// Creates a spectrum from sampled data.
        /// </summary>
        public static Spectrum FromSampled(float[] wavelengths, float[] values)
        {
            // Sort samples using wavelength as the key.
            Array.Sort(wavelengths, values);

            var n = wavelengths.Length;
            float x = 0.0f, y = 0.0f, z = 0.0f;
            float yint = 0.0f;
            for (var i = 0; i < SpectrumData.NumCieSamples; i++)
            {
                yint += SpectrumData.CieY[i];
                var val = InterpolateSpectrumSamples(wavelengths, values, n, SpectrumData.CieLambda[i]);
                x += val * SpectrumData.CieX[i];
                y += val * SpectrumData.CieY[i];
                z += val * SpectrumData.CieZ[i];
            }
            return FromXyz(x / yint, y / yint, z / yint);
        }

        /// <summary>
        /// Creates a spectrum from XYZ data.
        /// </summary>
        public static Spectrum FromXyz(float x, float y, float z)
        {
            var rgb = new float[3];
            XyzToRgb(new[] { x, y, z }, rgb);
            return new Spectrum(rgb);
        }

        /// <summary>
        /// Spectrum(0.0f, 0.0f, 0.0f)
        /// </summary>
        public static Spectrum CreateBlack()
        {
        ...
        public static Spectrum operator +(Spectrum left, Spectrum right)
        {
            var coefficients = new float[3];
            for (var i = 0; i < NumSamples; i++)
                coefficients[i] = left._coefficients[i] + right._coefficients[i];
            return new Spectrum(coefficients);
        }
        ...
        public static Spectrum operator /(Spectrum left, float right)
        {
            var coefficients = new float[3];
            for (var i = 0; i < NumSamples; i++)
                coefficients[i] = left._coefficients[i] / right;
            return new Spectrum(coefficients);
        }
        ...
        public static Spectrum operator *(float left, Spectrum right)
        {
            var coefficients = new float[3];
            for (var i = 0; i < NumSamples; i++)
                coefficients[i] = left * right._coefficients[i];
            return new Spectrum(coefficients);
        }

        #endregion

        private static class SpectrumData
        {
            public const int NumCieSamples = 471;

            /// <summary>
            /// CIE X function values
            /// </summary>
            public static readonly float[] CieX =
            {
            ...
            /// </summary>
            public static readonly float[] CieY =
            {
            ...
            /// </summary>
            public static readonly float[] CieZ =
            {
            ...
            public static readonly float[] CieLambda =
            {
